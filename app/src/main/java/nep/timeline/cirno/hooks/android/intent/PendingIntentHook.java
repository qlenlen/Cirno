      ...
      protected void beforeMethod(MethodHookParam param) {
        synchronized (
            XposedHelpers.getObjectField(
                XposedHelpers.getObjectField(param.thisObject, "controller"), "mLock")) {
          if (XposedHelpers.getBooleanField(param.thisObject, "canceled")) return;
          ...
          AppRecord appRecord =
              AppService.get(pendingIntentKey.getPackageName(), pendingIntentKey.getUserId());

          if (appRecord == null || !appRecord.isFrozen()) return;
