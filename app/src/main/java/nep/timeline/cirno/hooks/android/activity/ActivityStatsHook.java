  ...
  public XC_MethodHook getTargetHook() {
    return new AbstractMethodHook() {
      ...
      protected void beforeMethod(MethodHookParam param) {
        ...
        if (activityObject == null) return;

        ActivityRecord record = new ActivityRecord(activityObject);

        ActivityRecord activityRecord =
            (event == UsageEvents.Event.ACTIVITY_RESUMED
                    || event == UsageEvents.Event.ACTIVITY_PAUSED)
                ? activityRecords.computeIfAbsent(record.getToken(), k -> record)
                : activityRecords.remove(record.getToken());
        ...
        else appRecord.getAppState().getActivities().remove(activityRecord.getToken());
